...
- **规范文件**：[`docs/auto-iteration-spec.md`](./docs/auto-iteration-spec.md)（v1.2）
...
- **全局优先级**：项目健康故障修复 → 技术债清理 → 样式精修 → P3 体验优化（收尾补全与测试补全均已完成）
- **阶段锁定**：品质优化收尾已全部验收通过，阶段锁定已解除
- **当前阶段**：项目已达生产就绪状态（上线验收标准 7 项全部达标），推进最终全场景终验与部署测试
...
- 单次调度总时长上限：4 小时；
- 当前基线进度：品质优化专项 100% 完成（P0 三项收尾任务已于 2026-07-09 验收通过）；后端覆盖率 97% 达标，P2 技术债清理完成；P2 测试补全已完成（15/15 页面全覆盖），P3 体验优化已基本完成（Token 无感刷新已实现、无障碍适配已完成、PixiJS 资源懒加载经评估为不适用）；上线验收标准 7 项全部达标，项目已达生产就绪状态；所有已完成功能不得重复开发。本周（2026-07-24 ~ 2026-07-27）持续推进技术债清理与样式精修：boss-game/brawl-game 多轮 DRY 重构（颜色族、数值表、半径族、HP 公式、纹理缓存、PROJECTILE_RADIUS 抽取等）、home.tsx QUICK_NAV_ITEMS 配置化、leaderboardApi 泛型合并、weapons 幂等控制、第十一轮样式优化（btn-press / page-header / empty-state / alert-error / medal-row-bar / milestone-badge 组件类抽象覆盖 8 个页面）；后端测试 731/731 通过，前端构建 864 模块通过
