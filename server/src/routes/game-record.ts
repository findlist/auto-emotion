...
router.get('/', authMiddleware, async (req, res) => {
  // requireUser 与其他 13 个 routes 文件保持同一鉴权兜底范式，消除 req.user! 非空断言
  const user = req.user;
  ...
  const result = await recordService.listRecords(user.userId, page, pageSize);
  success(res, result);
});
...
router.get('/:id', authMiddleware, async (req, res) => {
  ...
  const record = await recordService.getRecord(recordId, user.userId);
  success(res, record);
});
