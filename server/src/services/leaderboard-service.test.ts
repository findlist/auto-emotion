...
describe('leaderboard-service 排行榜服务', () => {
  ...
  describe('getLeaderboard 榜单查询', () => {
    it('power 类型按 power 字段排序与分页 rank 计算', async () => {
      mocks.queryMock
        .mockResolvedValueOnce({ // 榜单查询
          rows: [
            { user_id: 1, nickname: 'A', score: 100 },
            { user_id: 2, nickname: 'B', score: 90 },
          ],
        })
      ...
      expect(result.ranking[0]).toEqual({ rank: 11, userId: 1, nickname: 'A', score: 100 });
      expect(result.ranking[1]).toEqual({ rank: 12, userId: 2, nickname: 'B', score: 90 });
    ...
    it('battle 类型映射 battle_score 字段', async () => {
      mocks.queryMock
        .mockResolvedValueOnce({ rows: [{ user_id: 1, nickname: 'A', score: 200 }] })
    ...
    it('speed 类型映射 speed_score 字段', async () => {
      mocks.queryMock
        .mockResolvedValueOnce({ rows: [{ user_id: 1, nickname: 'A', score: 50 }] })
        .mockResolvedValueOnce({ rows: [{ total: '1' }] });
    ...
    it('score 为 null 兜底为 0', async () => {
      mocks.queryMock
        .mockResolvedValueOnce({ rows: [{ user_id: 1, nickname: 'A', score: null }] })
        .mockResolvedValueOnce({ rows: [{ total: '1' }] });
  ...
  describe('getFriendsLeaderboard 好友榜单', () => {
    it('包含好友与自己，按 power 排序', async () => {
      // 好友列表查询
      mocks.queryMock
        .mockResolvedValueOnce({ rows: [{ friend_id: 2 }, { friend_id: 3 }] })
        // 好友排行查询
        .mockResolvedValueOnce({
          rows: [
            { user_id: 1, nickname: '我', score: 300 },
            { user_id: 2, nickname: '好友A', score: 200 },
          ],
        });
      ...
      expect(result.ranking[0]).toEqual({ rank: 1, userId: 1, nickname: '我', score: 300 });
      // 验证 ANY 参数包含自己（parseInt('1') = 1）与好友
    ...
    it('无好友仅自己，total=1', async () => {
      mocks.queryMock
        ...
        .mockResolvedValueOnce({ rows: [{ user_id: 1, nickname: '我', score: 100 }] });

      const result = await getFriendsLeaderboard('1', 1, 20);

      expect(result.total).toBe(1);
      expect(result.ranking[0]).toEqual({ rank: 1, userId: 1, nickname: '我', score: 100 });
    });

    it('score 为 null 兜底为 0', async () => {
      mocks.queryMock
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [{ user_id: 1, nickname: '我', score: null }] });
